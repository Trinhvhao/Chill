(function () {
  ...
  var scrapbookFlowers = document.getElementById("scrapbook-flowers");
  ...
  function initScrapbook() {
    ...
    // Caption fade-in: mark visible pages so .caption becomes visible
    function updateVisibleCaptions() {
      try {
        ...
        document.querySelectorAll(".page").forEach(function (pg) {
          pg.classList.remove("shown");
        });
        if (orientation === "landscape" && current > 0 && current < total - 1) {
          var left = current;
          var right = current + 1;
          var lp = pageFlip.getPageFromCollection ? pageFlip.getPageFromCollection(left) : null;
          var rp = pageFlip.getPageFromCollection ? pageFlip.getPageFromCollection(right) : null;
          // Fallback: just toggle via DOM order (each .page keeps original index in load order)
          var pages = document.querySelectorAll(".page");
          // In landscape spread, left page is index current (0-based on hard/soft),
          // but page-flip uses 0-based current page index where left=current and right=current+1.
          if (pages[left])  pages[left].classList.add("shown");
          if (pages[right]) pages[right].classList.add("shown");
        } else {
          var pages2 = document.querySelectorAll(".page");
          if (pages2[current]) pages2[current].classList.add("shown");
        }
      } catch (err) {}
    }

    // Gentle floating flowers drifting across scrapbook stage
    var scrapbookFlowerCounter = 0;
    function spawnScrapbookFlower() {
      if (!scrapbookFlowers) return;
      var w = scrapbookFlowers.clientWidth || window.innerWidth;
      var h = scrapbookFlowers.clientHeight || window.innerHeight;
      var img = document.createElement("img");
      img.src = FLOWER_CYCLE[scrapbookFlowerCounter % FLOWER_CYCLE.length];
      img.alt = "";
      img.className = "bloom-flower in";
      img.draggable = false;
      ...
      var startY = -size - Math.random() * 40;
      var endY = h + size + 20;
      var sway = (Math.random() - 0.5) * 120;
      var dur = 14 + Math.random() * 10;
      var delay = 0;
      var rstart = Math.round(Math.random() * 60 - 30);
      img.style.setProperty("--fx", startX + "px");
      img.style.setProperty("--fy", startY + "px");
      img.style.setProperty("--dur", dur + "s");
      img.style.setProperty("--delay", delay + "s");
      img.style.setProperty("--rstart", rstart + "deg");
      img.style.setProperty("--ffall", endY + "px");
      img.style.setProperty("--fsway", sway + "px");
      img.style.setProperty("--frot", (Math.round(Math.random() * 200 - 100) * 0.6) + "deg");
      img.style.setProperty("--fdur", dur + "s");
      img.style.setProperty("--fdelay", "0s");
      var hue = Math.round(Math.random() * 24 - 12);
      img.style.filter =
        "hue-rotate(" + hue + "deg) saturate(0.95) brightness(1.05) " +
        "drop-shadow(0 2px 4px rgba(120, 30, 40, 0.10))";
      img.style.zIndex = "0";
      img.style.opacity = "0.55";
      scrapbookFlowers.appendChild(img);
      scrapbookFlowerCounter++;

      // Trigger falling animation next frame
      requestAnimationFrame(function () {
        requestAnimationFrame(function () {
    ...
    function startScrapbookFlowerField() {
      if (!scrapbookFlowers) return;
      // Initial burst
      for (var i = 0; i < 5; i++) {
        setTimeout(spawnScrapbookFlower, i * 600);
      }
      // Continuous light drift
      setInterval(function () {
        spawnScrapbookFlower();
      }, 2200);
    }

    setTimeout(function () {
      updateVisibleCaptions();
      startScrapbookFlowerField();
    }, 200);
